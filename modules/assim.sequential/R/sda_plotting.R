...
post.analysis.multisite.ggplot <- function(settings, t, obs.times, obs.mean, obs.cov, FORECAST, ANALYSIS, plot.title=NULL, facetg=FALSE, readsFF=NULL){
  ...
  pdf(file.path(substr(settings$rundir, 1, nchar(settings$rundir)-4), "SDA.pdf"),width = filew, height = fileh)
  all.plots.print %>% purrr::map(~print(.x))
  dev.off()
  
  #saving plot data
  save(all.plots, ready.to.plot, file = file.path(substr(settings$rundir, 1, nchar(settings$rundir)-4), "timeseries.plot.data.Rdata"))
